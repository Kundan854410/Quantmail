...
describe("evaluateInboxRelevanceSync", () => {
  ...
  it("does not promote non-promotional mail even with matching keywords", () => {
    const result = evaluateInboxRelevanceSync(
      {
        senderEmail: "ceo@quantmail.com",
        subject: "Robotics project update",
        body: "The robotics arm is ready for testing",
      },
      [
        {
          videoTitle: "Advanced Robotics Workshop",
          watchedSeconds: 120,
          watchedAt: new Date("2026-04-01T09:00:00.000Z"),
        },
      ],
      TODAY
    );
  ...
  it("does not promote when watch event is too short (< 30 seconds)", () => {
    const result = evaluateInboxRelevanceSync(
      {
        senderEmail: "offers@gmail.com",
        subject: "Exclusive discount for robotics builders",
  ...
  it("does not promote when watch event is from a previous day", () => {
    const result = evaluateInboxRelevanceSync(
      {
        senderEmail: "offers@gmail.com",
        subject: "Exclusive discount for robotics builders",
